...
use crate::bootstrap::{ensure_self_venv, get_pip_module};
...
struct PipReport {
    install: Vec<PipDependencyReport>,
}
...
fn copy_constraints(lockfile: &Path, out: &Path) -> Result<(), Error> {
...
fn generate_lockfile(
    ...
    let constraints_file = scratch.path().join("requirements.txt");
    let report_file = scratch.path().join("report.json");
    copy_constraints(lockfile, &constraints_file)?;
    let self_venv = ensure_self_venv(output)?;
    symlink(get_pip_module(&self_venv), scratch.path().join("pip"))
        .context("failed linking pip module into for lock")?;

    // TODO: use a function that returns this.
    ...
    cmd.arg("-mpip")
        ...
        .env("PROJECT_ROOT", workspace_path)
    ...
    // XXX: this does not work with pip.  Should probably use unearth
    ...
    finalize_lockfile(&report_file, lockfile, workspace_path, exclusions)?;
...
fn finalize_lockfile(
    ...
    let report: PipReport = serde_json::from_slice(&fs::read(generated)?)?;
    ...
    for dep in &report.install {
        ...
        {
            let url =
                Url::parse(&dep.download_info.url).context("invalid editable URL generated")?;
            if url.scheme() == "file" {
            ...
            if exclusions.iter().any(|x| {
                normalize_package_name(&x.name) == normalize_package_name(&dep.metadata.name)
                    && (x.version_or_url.is_none()
                        || x.version_or_url.as_ref().map_or(false, |x| match x {
                            pep508_rs::VersionOrUrl::VersionSpecifier(v) => {
                                if let Ok(ver) = dep.metadata.version.parse() {
                                    v.contains(&ver)
                                } else {
                                    false
                                }
                            }
                            pep508_rs::VersionOrUrl::Url(_) => false,
                        }))
            }) {
                // skip exclusions
                writeln!(
                ...
                writeln!(rv, "{}=={}", dep.metadata.name, dep.metadata.version)?;
            }
        }
    }
    Ok(())
