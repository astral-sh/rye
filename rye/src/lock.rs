...
#   prerelease: {{ lock_options.prerelease|tojson }}
    ...
    pub fn restore<'o>(s: &str, opts: &'o LockOptions) -> Result<Cow<'o, LockOptions>, Error> {
            ...
            .skip_while(|x| *x != "# last locked with the following flags:")
        {
            if let Some(m) = PARAM_RE.captures(line) {
                let value = &m[2];
                match &m[1] {
                    "prerelease" => rv.prerelease = rv.prerelease.or(serde_json::from_str(value)?),
                    "features" => {
...
fn generate_lockfile(
    ...
    let mut cmd = if use_uv {
        ...
        if let Some(it) = lock_options.prerelease.as_ref() {
        ...
        // TODO(cnpryer)
        if lock_options.prerelease.is_some() {
            cmd.arg("--pre");
        }
        cmd
    };
