...
#[test]
fn test_add_flask_w_venv() {
    let venv_path = ".xyz";
    let space = Space::with_venv(venv_path);
    space.init("my-project");
    // add colorama to ensure we have this as a dependency on all platforms
    rye_cmd_snapshot!(space.rye_cmd().arg("add").arg("flask").arg("colorama").arg("--venv").arg(venv_path), @r###"
    success: true
    exit_code: 0
    ----- stdout -----
    Initializing new virtualenv in [TEMP_PATH]/project/.xyz
    Python version: cpython@3.12.3
    Added flask>=3.0.0 as regular dependency
    Added colorama>=0.4.6 as regular dependency
    Reusing already existing virtualenv
...
#[test]
fn test_add_flask_dotenv_w_venv() {
    let venv_path = ".xyz";
    let space = Space::with_venv(venv_path);
    space.init("my-project");
    // add colorama to ensure we have this as a dependency on all platforms
    rye_cmd_snapshot!(space.rye_cmd().arg("add").arg("flask[dotenv]").arg("colorama").arg("--venv").arg(venv_path), @r###"
    success: true
    exit_code: 0
    ----- stdout -----
    Initializing new virtualenv in [TEMP_PATH]/project/.xyz
...
#[test]
fn test_add_from_find_links_w_venv() {
    let venv_path = ".xyz";
    let space = Space::with_venv(venv_path);
    space.init("my-project");
    space.edit_toml("pyproject.toml", |doc| {
    ...
    rye_cmd_snapshot!(space.rye_cmd().arg("add").arg("tqdm").arg("colorama").arg("--venv").arg(venv_path), @r###"
    success: true
    exit_code: 0
    ----- stdout -----
    Initializing new virtualenv in [TEMP_PATH]/project/.xyz
...
#[test]
fn test_add_flask_wrong_venv_exported_w_venv() {
    let venv_path = ".xyz";
    let space = Space::with_venv(venv_path);
    space.init("my-project");
    let fake_venv = space.project_path().join("fake-venv");
    fs::create_dir_all(&fake_venv).unwrap();
    // add colorama to ensure we have this as a dependency on all platforms
    rye_cmd_snapshot!(space.rye_cmd().arg("add").arg("flask==3.0.0").arg("colorama").arg("--venv").arg(venv_path).env("VIRTUAL_ENV", fake_venv.as_os_str()), @r###"
    success: true
    exit_code: 0
    ----- stdout -----
    Initializing new virtualenv in [TEMP_PATH]/project/.xyz
...
#[test]
fn test_add_explicit_version_or_url_w_venv() {
    let venv_path = ".xyz";
    let space = Space::with_venv(venv_path);
    space.init("my-project");
    rye_cmd_snapshot!(space.rye_cmd().arg("add").arg("werkZeug==3.0.0").arg("--venv").arg(venv_path), @r###"
    success: true
    exit_code: 0
    ----- stdout -----
    Initializing new virtualenv in [TEMP_PATH]/project/.xyz
    ...
    let pip_url = "https://github.com/pypa/pip/archive/1.3.1.zip#sha1=da9234ee9982d4bbb3c72346a6de940a148ea686";
    rye_cmd_snapshot!(space.rye_cmd().arg("add").arg("pip").arg("--url").arg(pip_url).arg("--venv").arg(venv_path), @r###"
    success: true
    exit_code: 0
    ----- stdout -----
    Added pip @ https://github.com/pypa/pip/archive/1.3.1.zip#sha1=da9234ee9982d4bbb3c72346a6de940a148ea686 as regular dependency
...
#[test]
fn test_add_dev_w_venv() {
    let venv_path = ".xyz";
    let space = Space::with_venv(venv_path);
    space.init("my-project");
    // add colorama to ensure we have this as a dependency on all platforms
    rye_cmd_snapshot!(space.rye_cmd().arg("add").arg("flask").arg("colorama").arg("--dev").arg("--venv").arg(venv_path), @r###"
    success: true
    exit_code: 0
    ----- stdout -----
    Initializing new virtualenv in [TEMP_PATH]/project/.xyz
