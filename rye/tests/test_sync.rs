...
#[test]
fn test_empty_sync_w_venv() {
    let venv_path = ".xyz";
    let space = Space::with_venv(venv_path);
    space.init("my-project");
    let mut binding = space.rye_cmd();
    let mut cmd = binding.arg("sync").arg("--venv").arg(venv_path);
    println!("{:?}", cmd);
    rye_cmd_snapshot!(cmd, @r###"
    success: true
    exit_code: 0
    ----- stdout -----
...
#[test]
fn test_add_and_sync_no_auto_sync_w_venv() {
    let venv_path = ".xyz";
    let space = Space::with_venv(venv_path);
    space.init("my-project");

    // add colorama to ensure we have this as a dependency on all platforms
    rye_cmd_snapshot!(space.rye_cmd().arg("add").arg("flask==3.0.0").arg("colorama").arg("--no-sync").arg("--venv").arg(venv_path), @r###"
    success: true
    exit_code: 0
    ----- stdout -----
    Initializing new virtualenv in [TEMP_PATH]/project/.xyz
...
#[test]
fn test_add_autosync_w_venv() {
    let venv_path = ".xyz";
    let space = Space::with_venv(venv_path);
    space.init("my-project");
    // add colorama to ensure we have this as a dependency on all platforms
    rye_cmd_snapshot!(space.rye_cmd().arg("add").arg("flask==3.0.0").arg("colorama").arg("--venv").arg(venv_path), @r###"
    success: true
    exit_code: 0
    ----- stdout -----
    Initializing new virtualenv in [TEMP_PATH]/project/.xyz
...
#[test]
fn test_autosync_remember_w_venv() {
    // remove the dependency source markers since they are instable between platforms
    let mut settings = Settings::clone_current();
    settings.add_filter(r"(?m)^\s+# via .*\r?\n", "");
    settings.add_filter(r"(?m)^(\s+)\d+\.\d+s(   \d+ms)?", "$1[TIMING]");
    ...
    let space = Space::with_venv(venv_path);
    space.init("my-project");
    rye_cmd_snapshot!(space.rye_cmd().arg("sync").arg("--with-sources").arg("--all-features").arg("--venv").arg(venv_path), @r###"
    success: true
    exit_code: 0
    ----- stdout -----
    Initializing new virtualenv in [TEMP_PATH]/project/.xyz
    ...
    rye_cmd_snapshot!(space.rye_cmd()
        .arg("add").arg("--optional=web").arg("flask==3.0.0").arg("colorama").arg("--venv").arg(".xyz"),
        @r###"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    rye_cmd_snapshot!(space.rye_cmd().arg("add").arg("urllib3").arg("--venv").arg(".xyz"),
        @r###"
    success: true
    exit_code: 0
    ----- stdout -----
    ...
    // on CI we seem to have some flakage on this command with regards to
    // rebuilding the editable.
    rye_cmd_snapshot!(space.rye_cmd().arg("sync").arg("-q").arg("--venv").arg(".xyz"), @r###"
    success: true
    exit_code: 0
    ----- stdout -----
